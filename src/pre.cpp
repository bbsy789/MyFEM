...
{    
    ...
    MATRIX* Component_TSM(_IN ELEMENT_STACKS* element_stacks, _OUT ERROR_ID* errorID , _OUT MATRIX_STACKS* S)
    {
            ...
            element_node_ptr = element_stacks->Node;
            ...
            //提取右下角3*3矩阵Kjj元素的数组下标即为：6 * (i + 3 - 1) + (j + 3) - 1 = 6 * i + j + 14
            //提取总体刚度矩阵的数组下标即为：(3 * NW) * (B - 1) + C - 1
            //式中:
            //当取左上角分块矩阵Ki_indexi_index时：B = (3 * i_index - 3 + i) , C = (3 * i_index - 3 + j)
            //当取右上角分块矩阵Ki_indexj_index时：B = (3 * j_index - 3 + i) , C = (3 * i_index - 3 + j)
            //当取左下角分块矩阵Kj_indexi_index时：B = (3 * i_index - 3 + i) , C = (3 * j_index - 3 + j)
            //当取右下角分块矩阵Kj_indexj_index时：B = (3 * j_index - 3 + i) , C = (3 * j_index - 3 + j)
            //分别带入后可得，映射函数为：
            //Ki_indexi_index:NW * (9 * i_index + 3 * i - 12) + 3 * i_index + j - 4
            //Ki_indexj_index:NW * (9 * j_index + 3 * i - 12) + 3 * i_index + j - 4
            //Kj_indexi_index:NW * (9 * i_index + 3 * i - 12) + 3 * j_index + j - 4
            //Kj_indexj_index:NW * (9 * j_index + 3 * i - 12) + 3 * j_index + j - 4           
